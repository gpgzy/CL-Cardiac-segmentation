...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx in tbar:
            ...
            if self.mode == 'supervised':
                ...
                # print(next(dataloader)) next(dataloader)返回getitem返回的值
            ...
            if self.mode == 'supervised':
                ...
                # print(self.mode)
                # input_l = input_l.reshape(8,3,320,320)
                # target_l = target_l.reshape(8,3,320,320)
                # target_l = target_l.reshape(4,512,512)
                # target_l = target_l[:,0]
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(tbar):
                ...
                # output = output.reshape(1,4,512,512)
                # LOSS
                # print(output.shape)
                # print(target.shape)
                target = target.reshape(1,512,512)
                # target = target[:,:8,:512]
                # loss = F.cross_entropy(output, target.long(), ignore_index=self.ignore_index)
                ...
                correct, labeled, inter, union = eval_metrics(output, target, self.num_classes, 255)
                ...
                mIoU = IoU.mean()
                seg_metrics = {"Pixel_Accuracy": np.round(pixAcc, 3),
    ...
    def _generate(self,output,i,epoch):
        output = output.reshape(4, 512, 512)
        output = torch.max(output, dim=0)[1]
        output = output.to(dtype=torch.float)
        # print(output)
        output = output / 255
        image_p = transforms.ToPILImage()(output)
        # print(image_p)
        image_p = self._imageEhance(image_p)
        # print(image_p[image_p!=0])
        image_p.save('jpg/'+str(i)+'_'+str(epoch)+'_4CH'+'.png','PNG')
