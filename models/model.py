...
class CAC(BaseModel):
    def __init__(self, num_classes, conf, sup_loss=None, ignore_index=None, testing=False, pretrained=True):
        ...
        assert self.layers in [50, 101, 18]
        ...
        if self.backbone == 'deeplab_v3+':
            ...
            self.encoder = UNet(3,4)
            # self.out_dim = 64
            # self.classifier = nn.Sequential(nn.Dropout(0.1),nn.Conv2d(self.out_dim, num_classes, kernel_size=1, stride=1))
            self.classifier = nn.Sequential(nn.Dropout(0.1), nn.Conv2d(self.out_dim, num_classes, kernel_size=1, stride=1))
            # self.classifier = nn.Sequential(nn.Dropout(0.1), nn.Conv2d(64, num_classes, kernel_size=1, stride=1))
            for m in self.classifier.modules():
                if isinstance(m, nn.Conv2d):
                    torch.nn.init.kaiming_normal_(m.weight)
                elif isinstance(m, nn.BatchNorm2d):
        ...
        if self.mode == 'semi':
            self.project = nn.Sequential(
                ...
                # nn.BatchNorm2d(self.proj_final_dim)
            )
            #simCLR里面这个东西在imagenet上面提升了10个点
    ...
    def forward(self, x_l=None, target_l=None, x_ul=None, target_ul=None, curr_iter=None, epoch=None, gpu=None, gt_l=None, ul1=None, br1=None, \
        ...
        if self.mode == 'supervised':
            ...
            # print(target_l.shape)# 4 320 320
            ...
            #print(output_feature_list1)
            output_feat1 = torch.cat(output_feature_list1, 0) #[n, c]
            #print(output_feat1.shape)
    ...
    def get_other_params(self):
        if self.mode == 'supervised':
            ...
            # print(len(list(self.encoder.get_backbone_params()))+len(list(self.encoder.get_module_params()))+len(list(self.classifier.parameters()))+len(list(self.project.parameters())))
    ...
    def show_feature_map(self,feature_map):
        ...
        for index in range(1, feature_map_num + 1):
            ...
            plt.imshow(feature_map[0][index - 1], cmap='viridis')
            plt.axis('off')
