...
class BaseTrainer:
    def __init__(self, model, resume, config, iters_per_epoch, train_logger=None, gpu=None, test=False):
        ...
        # print(len(list(filter(lambda p:p.requires_grad, self.model.get_other_params()))))
        # print(len(list(filter(lambda p:p.requires_grad, self.model.get_backbone_params()))))
        ...
        # self.optimizer = torch.optim.Adam(trainable_params, lr=0.001,weight_decay=1e-8)
